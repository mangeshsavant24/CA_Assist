...
export const RegimeCalculator: React.FC = () => {
  ...
  return (
    <div className="min-h-full bg-[#050505] px-4 py-6">
      ...
      <div className={cn(
        "mx-auto flex flex-col md:flex-row gap-6 transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)]",
        (activeMode === 'regime' ? result : capitalResult) || loading
          ? "max-w-5xl" 
          : "max-w-[384px]" 
      )}>
        {/* Left Panel - Input */}
        ...
        {loading && (
          <div className="flex-1 space-y-4 animate-in slide-in-from-right-12 fade-in duration-300">
            <div className="h-[100px] bg-slate-900 border border-slate-800 rounded-xl shadow-xl animate-pulse" />
            <div className="grid grid-cols-2 gap-4">
              <div className="h-[380px] bg-slate-900 border border-slate-800 rounded-xl shadow-lg animate-pulse delay-75" />
              <div className="h-[380px] bg-slate-900 border border-slate-800 rounded-xl shadow-lg animate-pulse delay-100" />
            </div>
            <div className="grid flex-1 grid-cols-1 md:grid-cols-3 gap-4">
              <div className="h-[140px] bg-slate-900 border border-slate-800 rounded-xl animate-pulse delay-150" />
              <div className="h-[140px] bg-slate-900 border border-slate-800 rounded-xl animate-pulse delay-200" />
              <div className="h-[140px] bg-slate-900 border border-slate-800 rounded-xl animate-pulse delay-300" />
            </div>
          </div>
        )}

        {!loading && activeMode === 'regime' && result && (
          <div className="flex-1 space-y-4 animate-in slide-in-from-right-12 fade-in duration-300">
            {/* Verdict Banner */}
            <Card
              className={cn(
            ...
            {/* Mini-Chart (Visual Breakdown) */}
            <Card className="bg-slate-900 border-slate-700 overflow-hidden shadow-inner">
              <CardContent className="pt-5 relative">
                ...
                <div className="flex items-end gap-10 md:gap-16 h-24 px-4 md:px-8 border-b border-slate-800 pb-0">
                  <div className="w-1/2 bg-amber-500/20 rounded-t border-t border-amber-500/50 relative group transition-all duration-[1500ms] ease-[cubic-bezier(0.25,1,0.5,1)] hover:bg-amber-500/30" style={{ height: `${Math.max(10, (result.old_regime.total_tax / Math.max(result.old_regime.total_tax, result.new_regime.total_tax || 1)) * 100)}%` }}>
                  ...
                  <div className="w-1/2 bg-emerald-500/20 rounded-t border-t border-emerald-500/50 relative group transition-all duration-[1500ms] ease-[cubic-bezier(0.25,1,0.5,1)] hover:bg-emerald-500/30 delay-100" style={{ height: `${Math.max(10, (result.new_regime.total_tax / Math.max(result.old_regime.total_tax, result.new_regime.total_tax || 1)) * 100)}%` }}>
        ...
        {!loading && activeMode === 'capital' && capitalResult && (
          <div className="flex-1 space-y-4 animate-in slide-in-from-right-12 fade-in duration-300">
            <Card className="bg-slate-900 border-slate-700">
              <CardHeader>
